...
const State = {
  ...
  preset: 'general',
  isPro: false,
  aiHistory: [], // Last 3 turns: { role: 'user'|'assistant', content: '' }
  researchMode: false,
};
...
async function doSearch(q) {
  if (!q.trim()) return;
  State.query = q;
  State.researchMode = $('researchMode')?.checked || false;
  ...
  if (aiOverview) {
    ...
    aiOverview.innerHTML = `<div class="ai-loading"><div class="loader-ring"></div> Thinking...</div>`;
  }
  ...
  try {
    const response = await fetch('/.netlify/functions/ai-search', {
      ...
      body: JSON.stringify({
        query: q,
        history: State.aiHistory.slice(-6), // Last 3 turns = 6 messages
    ...
    if (data.answer) {
      if (aiOverview) {
        aiOverview.innerHTML = `<div class="ai-text" id="aiTextBody"></div><div class="ai-label">Generated by Clario AI</div>`;
        typeWriter($('aiTextBody'), data.answer);
      }
      // Update history
      State.aiHistory.push({ role: 'user', content: q });
      State.aiHistory.push({ role: 'assistant', content: data.answer });
      if (State.aiHistory.length > 6) State.aiHistory.shift(); State.aiHistory.shift();
    } else {
      throw new Error("No answer");
    }
  } catch (err) {
    if (aiOverview) aiOverview.innerHTML = `<div class="ai-text" style="color:var(--red)">Clario AI is temporarily unavailable.</div>`;
  }

  // Search Results Simulation (or real API if added later)
